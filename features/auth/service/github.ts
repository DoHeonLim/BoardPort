 ...
 * @returns {string} `-gh` 접미사가 포함된 최대 10자 기본 유저명
 */
function buildGitHubBaseUsername(login: string) {
  const baseUsername = `${login}-gh`.toLowerCase();
  return baseUsername.length > 10 ? baseUsername.slice(0, 10) : baseUsername;
}
...
export function isGitHubAutoGeneratedUsername(
  ...
  const collisionPrefix = `${baseUsername.slice(0, 5)}_`;
