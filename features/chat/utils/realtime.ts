...
/**
 * 여러 채팅방에 대해 실시간 이벤트를 구독합니다.
 *
 * [기능]
 * 1. 전달받은 roomIds 각각에 대해 Supabase 채널을 생성하고 구독합니다.
 * 2. `message` 이벤트: 내가 보낸 것이 아닌 새 메시지를 수신하면 onMessage 콜백을 실행합니다.
 * 3. `message_read` 이벤트: 상대방이 메시지를 읽으면 onMessageRead 콜백을 실행합니다.
 * 4. 구독 해제 함수(unsubscribe)를 반환하여 클린업을 지원합니다.
 *
