...
| 안정화와 문서화 | 2026.03 - 2026.06 | TanStack Query 기반 상태 정리, App Router 라우팅 안정화, 보드게임 도감, 릴리즈 QA, 데모 영상과 공개 문서 정리 |
...
- Server Component의 초기 조회와 prefetch는 service 계층 직접 호출
- Client Component queryFn의 재조회는 Route Handler fetch 사용
- 생성, 수정, 삭제, 토글 같은 사용자 의도 기반 변경은 Server Action 유지

이 기준을 잡은 뒤 초기 렌더 중 Server Function 호출 오류와 hydration 이후 재검증 경로 혼선이 줄었습니다.
...
### 실시간 데이터는 최종 상태가 아니라 재검증 신호

Realtime 이벤트는 UI를 바로 덮어쓰는 값이 아니라, 필요한 query를 invalidate/refetch하는 신호로 사용했습니다.

- 채팅방 목록과 TabBar 미읽음 수 재검증
- 방송 live-status는 상세 셸에서 한 번만 구독
...
| Supabase Realtime | 별도 WebSocket 서버를 직접 운영하기보다 채팅, 알림, 방송 상태 변경을 DB 기준 재검증 신호로 전파하기 위해 선택 |
| Prisma | 상품, 채팅, 약속, 방송, 신고처럼 관계가 많은 도메인을 타입 기반 query와 transaction으로 다루기 위해 선택 |
| Cloudflare Stream/Images | 방송/VOD와 이미지 업로드를 직접 인프라로 운영하지 않고, 외부 미디어 처리와 웹훅 기반 상태 동기화로 분리하기 위해 선택 |
...
- Realtime, Web Push, Cloudflare 웹훅처럼 비동기 이벤트가 많은 환경에서 DB 확정 상태를 기준으로 UI 동기화
