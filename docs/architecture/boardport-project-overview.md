...
| 사용 흐름과 검증 보강 | 2026.08 - 2026.09 | Next.js 16 전환, 서버 조회·캐시 경계 정리, 도감 연계 작성, 관심 목록 확장, 공통 UI와 반응형 배치 개선, 모바일 E2E 추가                             |
...
- 팔로우, 차단, 활동 뱃지, 프로필 수정
- 프로필에서 최근 작성 게시글을 확인하고 전체 작성글 목록으로 이동
- 관심 목록에서 상품·게시글·다시보기를 탭별로 조회하고 최근 찜한 순서로 탐색
- 관심 상품·게시글은 리스트/그리드 전환, 관심 다시보기는 그리드 제공

### Product Marketplace

- 보드게임 특화 상품 등록
- 지역 기반 탐색, 무한 스크롤, 검색/필터, 리스트/그리드 전환
- 최신순·가격 낮은순·가격 높은순 정렬과 URL 기반 탐색 상태 유지
- 판매중, 예약중, 판매완료 상태 전환
- 찜, 최근 본 상품, 거래 후기 연결
...
- 상세에서 상품·게시글·방송 작성으로 이동할 때 선택한 보드게임과 복귀 경로 전달
- 모바일 필터 시트와 이전·현재·다음 중심의 페이지 이동 제공
...
- 선택 입력 영역을 접고, 해당 필드의 검증 오류가 발생하면 영역을 열어 안내
...
- 미읽음 뱃지, 채팅방 목록 재검증
- 카카오맵 기반 직거래 장소 제안
- 달력과 시간 선택을 분리한 약속 일정 입력 및 과거 시각 선택 방지
...
- 공개, 팔로워, 비공개 방송 접근 제어
- 방송 종료 후 VOD 아카이빙
- 방송 소유자의 방송 썸네일 교체·제거와 연결 자산 정리
- 녹화본별 제목·사용자 썸네일 수정 및 사용자·자동·방송 썸네일 순서의 fallback
...
- 작성·수정 화면의 헤더 구조는 `SubpageHeader`로 공유하고 복귀 동작은 각 화면에서 전달
- 목록 보기 버튼은 `ViewModeToggle`로 공유하고 선택값은 URL에 반영
- `OptionalFormSection`으로 선택 입력을 접고 외부 검증 상태에서 펼침 제어
