...
- 작성·수정 화면은 공통 헤더 구조를 사용하되 각 화면의 복귀 경로 유지
- 태그와 위치는 선택 입력 영역에 배치하고 기존 입력값이 있으면 초기 펼침 상태에 반영
- 선택 입력의 검증 오류는 영역을 펼친 뒤 관련 입력으로 포커스 이동
- 제출 중에는 진행 문구와 비활성 상태로 중복 제출 방지

### 목록의 대표 이미지

리스트와 그리드는 같은 게시글 데이터를 사용하고 보기 방식은 URL에 반영합니다. 대표 이미지가 없으면 대체 상태를 표시하며 리스트의 이미지 영역이 높이 0으로 축소되지 않도록 카드 높이에 맞춥니다. 대표 이미지와 연결 보드게임 배지는 카드의 정보 영역과 구분해 배치합니다.
