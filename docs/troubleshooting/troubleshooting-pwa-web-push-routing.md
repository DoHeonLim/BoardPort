...
### 4.8 추가 정합성 점검에서 발견한 버그

코드와 정책을 대조하는 정합성 점검에서 아래 버그를 추가로 확인하고 수정했습니다.
...
### 6.1 앱 활성 상태에 따라 중복 노출 제어

같은 이벤트가 In-App과 Push 양쪽에 표시되는 경우가 있어, 앱 활성 상태와 현재 채팅방에 따라 노출 여부를 구분했습니다.

### 6.2 In-App과 Push의 노출 정책 분리

둘 다 같은 이벤트를 다루지만, 노출 정책까지 같을 필요는 없었습니다.

### 6.3 `tag`로 알림 누적과 교체 구분

Notification API의 `tag` 값에 따라 알림을 누적하거나 기존 알림을 교체하므로, 알림 유형별로 값을 구분합니다.
...
알림 생성, In-App 표시, Push 전송, 클릭 후 이동의 처리 경로를 분리했습니다. 채널별 노출 조건과 `tag`로 중복 표시를 제어하고, 알림 클릭 시 기존 탭을 우선 재사용합니다. 배포 후 오류는 실행 중인 Service Worker 버전과 캐시 상태를 함께 확인합니다.
