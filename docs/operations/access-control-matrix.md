...
BoardPort의 접근 제어는 `proxy.ts` 한 곳에만 의존하지 않고, 페이지 진입, Route Handler, Server Action, service 계층에서 도메인별로 다시 확인합니다.

주요 사용자 역할과 보호 지점을 페이지, Route Handler, Server Action, 외부 이벤트 경계별로 정리합니다. 세부 권한 판단은 각 도메인의 `actions`, `service`, `route.ts` 구현을 기준으로 합니다.
...
| 캐시와 권한 분리      | TanStack Query key는 사용자별 조회 결과를 캐시에서 구분합니다. 권한은 서버 세션과 DB 상태를 기준으로 판단합니다. |
...
| 앱 영역           | `/products`, `/posts`, `/chat`, `/profile`, `/streams` 등 | `proxy.ts`에서 로그인 세션 확인 후 비로그인 사용자는 `/login?callbackUrl=...`로 이동                                 |
| 관리자 영역       | `/admin/*`                                                | 관리자 권한 확인. 일반 사용자는 관리자 화면에 남을 수 없음                                                           |
| API Route Handler | `/api/*`                                                  | `proxy.ts` matcher에서 제외되므로 공개 조회 handler는 입력을 검증하고, 개인화/변경 handler는 세션과 권한을 직접 확인 |
...
| 행위           | 허용 대상                        | 대표 가드                                        | 비고                                                           |
| -------------- | -------------------------------- | ------------------------------------------------ | -------------------------------------------------------------- |
| 채팅방 생성    | 상품 작성자가 아닌 로그인 사용자 | 세션, 상품 존재 여부, 정지 상태, 차단 관계       | 상품·구매자 DB 고유 키와 upsert로 다중 인스턴스 중복 생성 방어 |
...
| 관리자 화면 접근 | 관리자        | `proxy.ts` 및 서버 측 권한 확인         | 일반 계정의 관리자 화면 접근 차단 E2E로 확인                       |
...
- `/api/*`가 `proxy.ts` 보호를 받는다고 가정하지 않았는가?
