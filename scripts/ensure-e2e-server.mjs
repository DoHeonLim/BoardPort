 ...
 * Description : Playwright 실행 전 대상 BoardPort 서버 준비 여부 확인
 * Author : 임도헌
 *
 * History
...
try {
  ...
  console.error("");
  console.error("[E2E] 대상 BoardPort 서버에 연결할 수 없습니다.");
  console.error("");
  console.error(
    "Playwright E2E는 실행 중인 BoardPort 서버를 대상으로 검증합니다."
  );
  console.error("아래 순서로 실행해주세요.");
  console.error("");
  console.error("1. 터미널 1에서 서버 실행");
  console.error("   빠른 로컬 반복: npm run dev:e2e");
  console.error("   production 검증: npm run build && npm run start:e2e");
  console.error("");
  console.error("2. 터미널 2에서 E2E 실행");
  console.error("   npm run test:e2e -- --project=chromium");
  console.error("");
  console.error(
    "seed 기반 테스트까지 실행할 때는 tests/e2e/README.md 기준으로"
  );
