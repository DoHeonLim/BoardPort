...
import AdminOverviewCharts from "@/features/report/components/admin/dashboard/AdminOverviewCharts";
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {quickLinks.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="inline-flex items-center rounded-full border border-border bg-surface px-3 py-2 text-xs font-semibold text-primary transition-colors hover:bg-surface-dim"
            >
              {item.label}
            </Link>
          ))}
